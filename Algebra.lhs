...
I got this problem for homework when I was studying abstract algebra,
but I didn't want to compute 36 function compositions by hand,
so I wrote this program to answer the question for me.
It has some sentimental value for me because it was
the first nontrivial Haskell program I wrote.

I think it's a cute program, but I wasn't sure if anyone doing
Hacker School admissions reads Haskell, so I decided to make use of some of
the reading I've been doing about literate programming. Enjoy!
...
because the functions I called f1 to f6 are traditionally named phi_1 to phi_6,
and I want f_n to be at index n in my list of permutations.
...
(output !! n) contains the image of f_n applied to [1, 2, 3].
I make use of this correspondence below.
...
two functions are equal if and only if for every input they give the same output.
I use this strategy.
...
interactively until interrupted, so that I could fill in a few answers at a time.

main asks for two numbers, which it interprets as elements of S_3.
It composes them, applies the resulting function to the input [1, 2, 3], and then
finds our answer as the index where this result can already be found in "output".
This works because of the correspondence between indices of "output"
and "permutations", mentioned above.
...
I enjoyed writing this, both code and commentary, and I hoped you liked it too!
